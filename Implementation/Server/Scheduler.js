...
async function scheduleDevice(device) {
    return new Promise(async (resolve, reject) => {
        ...
        if (surplusGraph.values[date.getMinutes()] - device.maxPower > 0 && device.nextState !== "on") {
        ...
        else if (surplusGraph.values[date.getMinutes()] - device.maxPower <= 0 && device.nextState !== "off") {
